 ...
 * Versão: 3.0 (Simplificada - Sem IIFE)
 */
...
function openModal(tipo, id = null) {
    console.log('🔓 Abrindo modal:', tipo, 'ID:', id);
    ...
    if (tipo === 'cliente') {
        ...
        if (!modal) {
            console.error('❌ Modal cliente não encontrado!');
            return;
        }
        ...
        if (modal) {
            ...
            console.log('✓ Modal técnico aberto');
        }
    } else if (tipo === 'zona') {
        ...
        tbody.innerHTML += `
            <tr class="border-b border-white/5 hover:bg-white/5 transition-colors">
                ...
                <td class="p-4 text-center">
                    <button onclick="editCliente(${c.id})" class="text-acqua-light hover:text-white mr-3 transition-colors" title="Editar">
                        <i class="fas fa-edit"></i>
                    </button>
                    <button onclick="openReagendar(${c.id})" class="text-yellow-400 hover:text-yellow-300 mr-3 transition-colors" title="Reagendar">
...
function renderAgenda() {
    ...
    appData.tecnicos.filter(t => t.status === 'disponivel').forEach(t => {
        ...
        html += `<div class="border border-white/10 rounded-lg p-4"><div class="flex justify-between items-center mb-3"><div class="flex items-center gap-2"><div class="w-4 h-4 rounded-full" style="background-color: ${t.cor}"></div><span class="font-semibold">${t.nome}</span></div></div></div>`;
    });
    html += '</div>';
...
function renderNaoAtribuidos() {
    ...
    naoAtribuidos.forEach(c => {
        container.innerHTML += `<div class="glass-effect rounded-xl p-4 border border-yellow-500/30"><h4 class="font-semibold">${c.nome}</h4><p class="text-sm text-gray-400">${c.bairro}</p></div>`;
    });
}
...
function renderZonas() {
    ...
    appData.zonas.forEach(z => {
        ...
        container.innerHTML += `<div class="glass-effect rounded-xl p-6 border-t-4" style="border-color: ${z.cor}"><h4 class="font-semibold">${z.nome}</h4><p class="text-sm text-gray-400 mb-3">${z.descricao}</p><span>${count} piscinas</span></div>`;
    });
}
...
function enviarAgendaTecnico(tecnicoId) {
    ...
    clientes.forEach((c, index) => {
        ...
        mensagem += `   📍 ${c.endereco}\n`;
        mensagem += `   🏘️ ${c.bairro}\n`;
        mensagem += `   ⚖️ ${c.peso} pts | ⏱️ ${c.tempo} min\n\n`;
    ...
    mensagem += '━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    mensagem += `*Resumo:* ${clientes.length} piscinas • ${totalPontos.toFixed(1)} pts • ${totalTempo} min\n`;
    mensagem += `🔗 Acesse: ${window.location.origin}/worker.html?tecnico=${tecnicoId}\n`;

    if (typeof WhatsAppIntegration !== 'undefined') {
        WhatsAppIntegration.compartilharWhatsApp(mensagem);
    } else {
        alert('Compartilhe esta mensagem:\n\n' + mensagem);
    }
}
...
console.log('✓ app-simple.js carregado com sucesso');
